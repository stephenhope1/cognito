...
# Determine the absolute root of the project to ensure safe file operations.
# Structure assumption: core/tools.py -> core/ -> root/
PROJECT_BASE_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))


# --- Tool Manifest ---
# This dictionary defines the "menu" of capabilities available to the Planner AI.
# The Planner uses these descriptions to decide which tool to allocate for a step.
TOOL_MANIFEST = [
    # 1. NATIVE TOOLS (Executed via specific API logic in main.py)
    {
    ...
    {
        "tool_name": "read_file",
        "description": "Reads the entire content of a specified text file from a safe directory and returns it as a string. Use this to get context from local user data files.",
        "parameters": [
...
def read_file_tool(filename: str) -> str:
    """
    Reads a file from the user's allowed data directories (inbox/output).
    Includes sandbox validation.
    """
    ...
    try:
        ...
        for safe_dir in allowed_dirs:
            ...
            # Sandbox Check: Ensure the path is strictly inside the safe_dir
        ...
        logger.error(f"Error reading file: '{filename}' not found or access denied.")
...
def read_internal_file_tool(filename: str) -> str:
    """
    Reads a source code file from the project structure.
    Strictly forbids access to data, logs, or credentials.
    """
    try:
        ...
        sensitive_paths = ['data/', 'logs/', '.env', '.git', 'node_modules', 'credentials.json', 'token.json']
...
def write_to_file_tool(filename: str, content: str) -> str:
    """
    Writes content to a file in the 'data/output' directory.
...
# Note: Native tools are handled separately in the orchestrator.
TOOL_EXECUTOR = {
